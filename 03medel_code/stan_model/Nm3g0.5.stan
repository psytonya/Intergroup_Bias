//U= Xself-beta*P-alpha·max((Xother-3*beta*P)-(Xself-beta*P),0)-theta·max((Xself-beta*P)-(Xother-3*beta*P),0) 
//alpha_temp+beta_temp+theta_temp
...
// Declare all parameters as vectors for vectorizing
  ...
  vector[Ns] beta_pr11;  // ingroup_beta:loss aversion
  vector[Ns] beta_pr12;   // outgroup_beta:loss distance
...
transformed parameters {
  ...
  real theta11[Ns];
  real theta12[Ns];
  real<lower=0> beta11[Ns];
  real<lower=0> beta12[Ns];
  real<lower=0> tau[Ns];
  ...
  for (i in 1:Ns) {
    ...
    beta11[i]  = exp(mu_pr[5] + sigma[5] * beta_pr11[i]);
    beta12[i]  = exp(mu_pr[6] + sigma[6] * beta_pr12[i]);
  ...
  for (i in 1:Ns) {
    ...
    for (t in 1:Tsubj[i]) { //loop over the trial number of each subject. Therefore, skipping the non-responsed trials.
     ...
     if(in_outgroup[i,t]==1 ){
      ...
      util[1] = offer_recipt[i,t]+6-0-alpha_temp*(fmax((offer_propoer[i,t]+6-3*0*beta_temp)-(offer_recipt[i,t]+6-0),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-0)-(offer_propoer[i,t]+6-3*0*beta_temp),0));// if chosing 0
      util[2] = offer_recipt[i,t]+6-2-alpha_temp*(fmax((offer_propoer[i,t]+6-3*2*beta_temp)-(offer_recipt[i,t]+6-2),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-2)-(offer_propoer[i,t]+6-3*2*beta_temp),0));// if chosing 2
      util[3] = offer_recipt[i,t]+6-4-alpha_temp*(fmax((offer_propoer[i,t]+6-3*4*beta_temp)-(offer_recipt[i,t]+6-4),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-4)-(offer_propoer[i,t]+6-3*4*beta_temp),0));// if chosing 4
      util[4] = offer_recipt[i,t]+6-6-alpha_temp*(fmax((offer_propoer[i,t]+6-3*6*beta_temp)-(offer_recipt[i,t]+6-6),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-6)-(offer_propoer[i,t]+6-3*6*beta_temp),0));// if chosing 6
...
generated quantities {
  ...
  real mu_theta11;
  real mu_theta12;
  real<lower=0> mu_beta11;
  real<lower=0> mu_beta12;
  real<lower=0> mu_tau;
  ...
  mu_beta11   = exp(mu_pr[5]);//损失厌恶
  mu_beta12   = exp(mu_pr[6]);//损失厌恶
