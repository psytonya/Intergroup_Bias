//U= Xself-gamma*P-alpha·max((Xother-3*gamma*P)-(Xself-gamma*P),0)-theta·max((Xself-gamma*P)-(Xother-3*gamma*P),0) 
//alpha_temp+gamma_temp+theta_temp
...
// Declare all parameters as vectors for vectorizing
  ...
  vector[Ns] gamma_pr11;  // ingroup_gamma:loss aversion
  vector[Ns] gamma_pr12;   // outgroup_gamma:loss distance
...
transformed parameters {
  ...
  real theta11[Ns];
  real theta12[Ns];
  real<lower=0> gamma11[Ns];
  real<lower=0> gamma12[Ns];
  real<lower=0> tau[Ns];
  ...
  for (i in 1:Ns) {
    ...
    gamma11[i]   = exp(mu_pr[5] + sigma[5] * gamma_pr11[i]);
    gamma12[i]   = exp(mu_pr[6] + sigma[6] * gamma_pr12[i]);
...
model {
  ...
  gamma_pr11  ~ normal(0, 1.0);
  gamma_pr12  ~ normal(0, 1.0);
  tau_pr      ~ normal(0, 1.0);
  ...
  for (i in 1:Ns) {
    ...
    for (t in 1:Tsubj[i]) { //loop over the trial number of each subject. Therefore, skipping the non-responsed trials.
     ...
     if(in_outgroup[i,t]==1 ){
        ...
        gamma_temp = gamma11[i];
        theta_temp= theta11[i];
      } else {
        alpha_temp = alpha12[i];
        gamma_temp = gamma12[i];
        theta_temp= theta12[i];
      }  

       
        // utility
      util[1] = offer_recipt[i,t]+6-0*gamma_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*0)-(offer_recipt[i,t]+6-0*gamma_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-0*gamma_temp)-(offer_propoer[i,t]+6-3*0),0));// if chosing 0
      util[2] = offer_recipt[i,t]+6-2*gamma_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*2)-(offer_recipt[i,t]+6-2*gamma_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-2*gamma_temp)-(offer_propoer[i,t]+6-3*2),0));// if chosing 2
      util[3] = offer_recipt[i,t]+6-4*gamma_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*4)-(offer_recipt[i,t]+6-4*gamma_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-4*gamma_temp)-(offer_propoer[i,t]+6-3*4),0));// if chosing 4
      util[4] = offer_recipt[i,t]+6-6*gamma_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*6)-(offer_recipt[i,t]+6-6*gamma_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-6*gamma_temp)-(offer_propoer[i,t]+6-3*6),0));// if chosing 6
...
generated quantities {
  ...
  real mu_theta11;
  real mu_theta12;
  real<lower=0> mu_gamma11;
  real<lower=0> mu_gamma12;
  real<lower=0> mu_tau;
  ...
  mu_gamma11  = exp(mu_pr[5]);//损失厌恶
  mu_gamma12  = exp(mu_pr[6]);//损失厌恶
