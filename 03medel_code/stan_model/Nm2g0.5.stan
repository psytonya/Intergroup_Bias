//U= Xself-lambda*P-alpha·max((Xother-3*lambda*P)-(Xself-lambda*P),0)-theta·max((Xself-lambda*P)-(Xother-3*lambda*P),0) 
//alpha_temp+lambda_temp+theta_temp
...
// Declare all parameters as vectors for vectorizing
  ...
  vector[7] mu_pr; //mean of the parameters,7 paras: punishment willingness??punishment degree and a inverse temperature.
  vector<lower=0>[7] sigma; //variance of the parameters, 7 paras
  ...
  vector[Ns] lambda_pr11;  // ingroup_lambda:loss aversion
  vector[Ns] lambda_pr12;   // outgroup_lambda:loss distance
...
transformed parameters {
  ...
  real theta11[Ns];
  real theta12[Ns];
  real<lower=0> lambda11[Ns];
  real<lower=0> lambda12[Ns];
  real<lower=0> tau[Ns];
  ...
  for (i in 1:Ns) {
    ...
    lambda11[i]  = exp(mu_pr[5] + sigma[5] * lambda_pr11[i]);
    lambda12[i]  = exp(mu_pr[6] + sigma[6] * lambda_pr12[i]);
    tau[i]       = exp(mu_pr[7] + sigma[7] * tau_pr[i]);
  }
}
...
model {
  ...
  lambda_pr11 ~ normal(0, 1.0);
  lambda_pr12 ~ normal(0, 1.0);
  tau_pr      ~ normal(0, 1.0);
  ...
  for (i in 1:Ns) {
    ...
    for (t in 1:Tsubj[i]) { //loop over the trial number of each subject. Therefore, skipping the non-responsed trials.
     ...
     if(in_outgroup[i,t]==1 ){
        ...
        lambda_temp = lambda11[i];
        theta_temp= theta11[i];
      } else {
        alpha_temp = alpha12[i];
        lambda_temp = lambda12[i];
        theta_temp= theta12[i];
      }  

      util[1] = offer_recipt[i,t]+6-0*lambda_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*0*lambda_temp)-(offer_recipt[i,t]+6-0*lambda_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-0*lambda_temp)-(offer_propoer[i,t]+6-3*0*lambda_temp),0));// if chosing 0
      util[2] = offer_recipt[i,t]+6-2*lambda_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*2*lambda_temp)-(offer_recipt[i,t]+6-2*lambda_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-2*lambda_temp)-(offer_propoer[i,t]+6-3*2*lambda_temp),0));// if chosing 2
      util[3] = offer_recipt[i,t]+6-4*lambda_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*4*lambda_temp)-(offer_recipt[i,t]+6-4*lambda_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-4*lambda_temp)-(offer_propoer[i,t]+6-3*4*lambda_temp),0));// if chosing 4
      util[4] = offer_recipt[i,t]+6-6*lambda_temp-alpha_temp*(fmax((offer_propoer[i,t]+6-3*6*lambda_temp)-(offer_recipt[i,t]+6-6*lambda_temp),0))
                -theta_temp*(fmax((offer_recipt[i,t]+6-6*lambda_temp)-(offer_propoer[i,t]+6-3*6*lambda_temp),0));// if chosing 6
...
generated quantities {
  ...
  real mu_theta11;
  real mu_theta12;
  real<lower=0> mu_lambda11;
  real<lower=0> mu_lambda12;
  real<lower=0> mu_tau;
  ...
  mu_lambda11 = exp(mu_pr[5]);//损失厌恶
  mu_lambda12 = exp(mu_pr[6]);//损失厌恶
  mu_tau      = exp(mu_pr[7]) ;//逆温
