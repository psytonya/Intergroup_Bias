...
3.7 set up model Nm1c
```{r set up model Nm3g}
ncore <- 3
options(mc.cores = ncore) #adapted from hBayesDM_model.R
rstan_options(javascript=FALSE)
#model m4, 3 parameters: alpha,beta,tau
pars_m1c <- c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_tau','sigma','alpha11','alpha12','theta11','theta12','tau','log_lik')
fit_m1c <- stan(file=paste0(getwd(),'/stan_model/Nm1c0.5.stan'), data=data_list,
pars = pars_m1c,
chains = 3,
iter = 12000,
warmup = 6000,
...
save(fit_m1c,file ='./stan_output/fit_Nm1c0.5.RData')#非常重要！！！一定要在本地路径建相应文件夹stan_output
printFit(fit_m1c,ic='both')
print(fit_m1c, pars = c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_tau','sigma')) #all are group-level parameters

#plot group parameters
stan_dens(fit_m1c, pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_tau','sigma')), separate_chains = T) #should overlap to each other
#plot individual parameters
stan_plot(fit_m1c, pars = 'alpha11', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m1c, pars = 'alpha12', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m1c, pars = 'theta11', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m1c, pars = 'theta12', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m1c, pars = 'tau', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
...
traceplot(fit_m1c, pars = c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_tau','sigma')) #all are group-level parameters # should be stationary and overylap to each other
mcmc_rank_hist(fit_m1c, pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_tau','sigma[1]','sigma[2]','sigma[3]','sigma[4]','sigma[5]')), ref_line = TRUE) # should be uniformly distributed ##A traditional traceplot (mcmc_trace()) visualizes how sampled values the MCMC chains mix over the course of sampling. A rank histogram (mcmc_rank_hist()) visualizes how the ranks of values from the chains mix together. An ideal plot would show the ranks mixing or overlapping in a uniform distribution##
...
rhats <- mcmc_rhat_data(rhat(fit_m1c)) # a tibble format
stan_rhat(fit_m1c)
#stan_rhat(fit_m1a)$data
mcmc_rhat(rhat(fit_m1c))

#1.3.energy diagnostic plot, E and deltaE should overlap each other.
mcmc_nuts_energy(nuts_params(fit_m1c))
...
neff_ratio <- mcmc_neff_data(neff_ratio(fit_m1c)) # a tibble format
stan_ess(fit_m1c)
mcmc_neff(neff_ratio(fit_m1c))

#2.2. auto correlations, should decrease with lag (larger correlation with small lag, but smaller correlation with large lag)
mcmc_acf(as.array(fit_m1c), pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_tau','sigma[1]','sigma[2]','sigma[3]','sigma[4]','sigma[5]')),inc_warmup = F)

#2.3. for total sample size, should be 4000.
length(extract(fit_m1c, pars = 'lp__')[[1]])


#3. for divergent transitions, n_treedepth, accept_stat (i.e., adapt_delta parameter), stepsize, and energy
summary(do.call(rbind,args = get_sampler_params(fit_m1c,inc_warmup = F)),digits=2) # across all chains所有链
lapply(get_sampler_params(fit_m1c,inc_warmup = F),summary,digits=2) # for each chain每条链分开

check_hmc_diagnostics(fit_m1c) # explicit checking of above parameters,including warmup transitions
...
mcmc_pairs(
as.array(fit_m1c),
pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_tau','sigma[1]','sigma[2]','sigma[3]','sigma[4]','sigma[5]')),
np = nuts_params(fit_m1c
                 ))
