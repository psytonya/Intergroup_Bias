...
4.7 set up model Nm4g
```{r set up model Nm4g}
ncore <- 3
options(mc.cores = ncore) #adapted from hBayesDM_model.R
rstan_options(javascript=FALSE)
#model m4, 3 parameters: alpha,gamma,tau
pars_m4g <- c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_gamma11','mu_gamma12','mu_tau','sigma','alpha11','alpha12','theta11','theta12','gamma11','gamma12','tau','log_lik')
fit_m4g <- stan(file=paste0(getwd(),'/stan_model/Nm4g0.5.stan'), data=data_list,
pars = pars_m4g,
chains = 3,
iter = 12000,
warmup = 6000,
...
save(fit_m4g,file ='./stan_output/fit_Nm4g0.5.RData')#非常重要！！！一定要在本地路径建相应文件夹stan_output
printFit(fit_m4g,ic='both')
print(fit_m4g, pars = c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_gamma11','mu_gamma12','mu_tau','sigma')) #all are group-level parameters

#plot group parameters
stan_dens(fit_m4g, pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_gamma11','mu_gamma12','mu_tau','sigma')), separate_chains = T) #should overlap to each other
#plot individual parameters
stan_plot(fit_m4g, pars = 'alpha11', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m4g, pars = 'alpha12', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m4g, pars = 'theta11', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m4g, pars = 'theta12', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m4g, pars = 'gamma11', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m4g, pars = 'gamma12', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
stan_plot(fit_m4g, pars = 'tau', ci_level = 0.95, outer_level = 1, show_density = T, point_est = 'mean')
...
#see also: https://gammanalpha.github.io/assets/case_studies/rstan_workflow.html
###1. Convergence and divergence
#1.1. overlaps between different chains
traceplot(fit_m4g, pars = c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_gamma11','mu_gamma12','mu_tau','sigma')) #all are group-level parameters # should be stationary and overylap to each other
mcmc_rank_hist(fit_m4g, pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_gamma11','mu_gamma12','mu_tau','sigma[1]','sigma[2]','sigma[3]','sigma[4]','sigma[5]','sigma[6]','sigma[7]')), ref_line = TRUE) # should be uniformly distributed ##A traditional traceplot (mcmc_trace()) visualizes how sampled values the MCMC chains mix over the course of sampling. A rank histogram (mcmc_rank_hist()) visualizes how the ranks of values from the chains mix together. An ideal plot would show the ranks mixing or overlapping in a uniform distribution##
...
rhats <- mcmc_rhat_data(rhat(fit_m4g)) # a tibble format
stan_rhat(fit_m4g)
#stan_rhat(fit_m1a)$data
mcmc_rhat(rhat(fit_m4g))

#1.3.energy diagnostic plot, E and deltaE should overlap each other.
mcmc_nuts_energy(nuts_params(fit_m4g))
...
neff_ratio <- mcmc_neff_data(neff_ratio(fit_m4g)) # a tibble format
stan_ess(fit_m4g)
mcmc_neff(neff_ratio(fit_m4g))

#2.2. auto correlations, should decrease with lag (larger correlation with small lag, but smaller correlation with large lag)
mcmc_acf(as.array(fit_m4g), pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_gamma11','mu_gamma12','mu_tau','sigma[1]','sigma[2]','sigma[3]','sigma[4]','sigma[5]','sigma[6]','sigma[7]')),inc_warmup = F)

#2.3. for total sample size, should be 4000.
length(extract(fit_m4g, pars = 'lp__')[[1]])


#3. for divergent transitions, n_treedepth, accept_stat (i.e., adapt_delta parameter), stepsize, and energy
summary(do.call(rbind,args = get_sampler_params(fit_m4g,inc_warmup = F)),digits=2) # across all chains所有链
lapply(get_sampler_params(fit_m4g,inc_warmup = F),summary,digits=2) # for each chain每条链分开

check_hmc_diagnostics(fit_m4g) # explicit checking of above parameters,including warmup transitions
...
mcmc_pairs(
as.array(fit_m4g),
pars = paste0(c('mu_alpha11','mu_alpha12','mu_theta11','mu_theta12','mu_gamma11','mu_gamma12','mu_tau','sigma[1]','sigma[2]','sigma[3]','sigma[4]','sigma[5]','sigma[6]','sigma[7]')),
np = nuts_params(fit_m4g
                 ))
